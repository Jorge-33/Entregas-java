...
public class Funciones {
	    ...
	    public static Integer P2(int n, int k, int i) {
	    	...
	    	try {
	    		if(i>=(k+1)) {
	    			 throw new IllegalArgumentException("i debe ser menor que k+1 para poder hacer la operación");
	    		}
	    		if(n<k) {
	    			throw new IllegalArgumentException("n debe ser mayor o igual que k para poder hacer la operación");
	    		}
	    ...
	    public static Double C2(int n, int k) {
	        ...
	        try {
	            if (n == 0 || k == (-1) || n < 0 || k < (-1)) {
	                throw new IllegalArgumentException("n no pede ser cero o negativo y k no puede ser ni igual ni menor a (-1)");
	            }
	            if (n < k) {
	                throw new IllegalArgumentException("n debe ser mayor que k para poder hacer la operación");
	            } else {
	                int h = n - (k+1);
	                double p = (fact(n)) / (fact(k+1) * fact(h));
	                return p;
	            }
	        } catch (IllegalArgumentException e) {
	            System.out.println("Se ha producido una excepción: " + e.getMessage());
	    ...
	    public static Double S2(int n, int k) {
	        ...
	        try {
	        	if(n<=k) {
		            ...
		            double resultado = (sumatorio * fact(k))/(fact(k+2));
		            return resultado;
	        }
	        } catch (IllegalArgumentException e) {
	            System.out.println("Se ha producido una excepción: " + e.getMessage());
