...
public class Fecha {
	...
	public Integer dia;
	  ...
	  // Apartado D
	  public static Integer[] restarDiasFechaDada(Fecha fecha, Integer numDias) {

		  try {
			  if(numDias<=999 && numDias<0) {
				  throw new IllegalArgumentException ("n debe ser positivo y menor que 999");  
			  }
			  else {
				  Integer p = fecha.dia - numDias;
				    while (p < 1) {
				        fecha.mes = fecha.mes - 1;
				        if (fecha.mes < 1) {
				            fecha.año = fecha.año - 1;
				    ...
				    fecha.dia = p;
				    Integer[] resultados = {fecha.año, fecha.mes, fecha.dia};
				    return resultados;
			  }
		  }
		  catch (IllegalArgumentException e) {
